...
Compatibilite legacy demandee:
- MySQL `5.5.16` peut fonctionner en mode degrade si vous appliquez le profil legacy ci-dessous.
- Attention: cette version est ancienne et non recommandee pour une nouvelle production.
...
### 3.1 Profil `.env` pour MySQL 5.5.16 (legacy)

Utiliser ces variables en plus des variables DB classiques:
...
Pourquoi:
- `utf8` / `utf8_unicode_ci`: evite certaines incompatibilites de collation anciennes.
- `DB_STRICT_MODE=false`: reduit les erreurs SQL strictes frequentes sur MySQL 5.5.
- `DB_ENGINE=InnoDB`: force un moteur transactionnel stable pour migrations/tables metier.
- `MYSQL_ATTR_LOCAL_INFILE=true`: necessaire pour les imports base sur `LOAD DATA LOCAL INFILE`.
